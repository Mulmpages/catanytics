from catanytics.ressources import Ressources
...
class Catan:
    ...
    def add_settlement(self, player, ressources):
        for r in ressources:
            self.settlements[self.turn][player][r] += ressources[r]
            # Can maybe remove the [r]?
    ...
    def calc_production(self):
        ...
        for p in self.players:
            for r in Ressources:
                s = self.get_settlements(self.turn, p, r)
                r = self.get_robber(self.turn, p, r)
                production[p][r] = s - r
